...
.stRadio > div {{
    ...
    margin-left: -183px !important;   /* pulls it closer to label */
}}
...
if submit:
    ...
    "status": status,
    "mode": mode,
    "shop_id": st.session_state.shop_id  # MUST BE HERE ✔
}).execute()
...
# LOAD DATA
# ==========================
if st.session_state.shop_id:
    response = supabase.table("sales") \
        .select("*") \
        .eq("shop_id", st.session_state.shop_id) \
        .execute()
else:
    st.warning("⚠ No shop selected. Login first.")
    st.stop()
